...
A WhatsApp food assistant agent for Indian users. Parses grocery invoice PDFs, tracks kitchen inventory, sends meal-time nudges, and suggests meals based on what you actually have — grounded in peer-reviewed nutrition science, not guesswork.

## What it does

- 📄 **Drop grocery bills** (Blinkit / Zepto / BigBasket PDFs) on WhatsApp → auto-builds kitchen inventory
...
- **TypeScript + Fastify + Drizzle ORM + PostgreSQL + Twilio WhatsApp**
...
Use ngrok (`ngrok http 3000`) to expose your local server to Twilio's WhatsApp sandbox webhook.
...
4. Add env vars in Railway dashboard (see `.env.example`).
5. Railway auto-detects the `railway.json` config.
6. Copy the public Railway URL → set as `PUBLIC_BASE_URL` → update Twilio webhook.
